          ...
          <div className="topbar-left">
            ...
            <h1 className="flex items-center gap-2 text-lg font-semibold text-[#3d2f26]">
              ...
              <div className="flex items-center ml-4">
                <button
                  type="button"
                  className={`relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 ${isOnline ? 'bg-primary-600' : 'bg-gray-300'}`}
                  role="switch"
                  aria-checked={isOnline}
                  onClick={() => setIsOnline(!isOnline)}
                >
                  <span className="sr-only">Toggle workspace status</span>
                  <span
                    aria-hidden="true"
                ...
                <span className={`ml-2 text-sm font-medium ${isOnline ? 'text-primary-600' : 'text-gray-500'}`}>
                  {isOnline ? 'On' : 'Off'}
                </span>
              </div>
            </h1>
