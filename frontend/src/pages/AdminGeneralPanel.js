...
const AdminGeneralPanel = () => {
  ...
  const refreshId = useCallback(async () => {
    try {
      ...
      if (msg) {
        setError(`Could not load a general ID. Server says: ${msg}`);
      } else {
        setError("Could not load a general ID. Sign in again and ensure backend is restarted.");
      }
    }
  }, []);
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      if (form._id) {
        ...
        setSuccess("General updated successfully.");
      } else {
        await distributorApi.create(payload);
        setSuccess("General created successfully.");
      }
      ...
      setError(err.response?.data?.message || `Could not ${form._id ? 'update' : 'create'} general.`);
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      setSuccess("General deleted successfully.");
      await loadList();
      if (form._id === id) {
        setForm(emptyForm());
        await refreshId();
      ...
      setError(err.response?.data?.message || "Could not delete general.");
    }
  };
  ...
  return (
    <div className="distributors-page mx-auto max-w-6xl">
      <header className="products-head mb-2">
        <div>
          <h2>General</h2>
          <p>Create general accounts aligned with compliance, payment terms, and contact records.</p>
        </div>
      </header>
      ...
      >
        ...
        <div className="flex flex-wrap justify-end gap-3 border-t border-[#ECEFF1] pt-6">
          ...
          <button
            ...
            disabled={submitting}
          >
            {submitting ? "Saving…" : form._id ? "Update General" : "Create General"}
          </button>
        </div>
      </form>
